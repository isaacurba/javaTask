...
public class OshoFreePromo {
    ...
    public static int calculateDiscount(int cartTotal, String promoCode){
        ...
        if (cartTotal >= 5000 && cartTotal <= 14999 && promoCode.toLowerCase().equalsIgnoreCase("STARTER10")){
            total = cartTotal - (cartTotal * 10 / 100);
        } 
        if (cartTotal >= 15000 && cartTotal <= 29999 && promoCode.toLowerCase().equalsIgnoreCase("BIGBOY20")){
            total = cartTotal - (cartTotal * 20 / 100);
        } 
        if (cartTotal >= 30000 && promoCode.toLowerCase().equalsIgnoreCase("OSHOFREE35")){
